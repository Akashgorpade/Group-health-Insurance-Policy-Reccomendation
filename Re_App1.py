...
import streamlit as st  ## importing the required package 
...
# Load our Dataset
df = pd.read_excel(r"D:\Project\Cluster_dataset2.xlsx")

tfidf = TfidfVectorizer(stop_words = "english")
tfidf_matrix = tfidf.fit_transform(df.Insurance_Provider_Name)
cosine_sim_matrix = linear_kernel(tfidf_matrix, tfidf_matrix)
    
# Recommendation sys # our class code
def get_recommendations(policy_select, num_of_recc=5):   
    ...
    result_df = df.iloc[policy_idx] # line number 31 which shows bout cosine similarity 
    final_recc = result_df[['Insurance_Provider_Name','Premium']] # To show the column name premium with insurance provider name 
    return final_recc

# CSS Style # to show the interface of the output
RESULT_TEMP = """
...
"""
img = Image.open("D:\Project\innodatatics.png")   # to show the image on top of the interface
st.image(img)
            ...
            if feature_name == "Premium":
                try:
                    ...
                    for row in results.iterrows():
                        ...
                        stc.html(RESULT_TEMP.format(rec_title, rec_prem), height=120) # to run the HTML
                    
                except:    
                    results = " "
                           
    else:
        st.subheader("Built with Streamlit")   # it says about the "About" feature
